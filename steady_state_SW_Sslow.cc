...
int main(int argc, char *argv[])
{
  ...
  std::string output_folder =
      "steady_state_SW_Sslow_" + coulomb_mu_text + "_" + std::to_string(nb_it_nodes) + "_" + damping_mode;
  ...
  const Real ramp_time = 0.5 * 0.5 / cs;
...
//   }
  ...
  for (UInt s = 0; s < nb_steps; ++s)
  {
    ...
    Real current_shear_vel = (rf <= 1.) ? shear_vel : 0.;
    ...
    for (UInt n = 0; n < nb_nodes; ++n)
    {
      ...
      displacement(n, 1) = rf * normal_strain_applied * position(n, 1);
