...
def main(filename):
    ...
    # Read data
    # -----------------------------
    reader_file = "/scratch/xyang/build-akantu/build/paraview/steady_state_" + filename + ".pvd"
    pd_file = "/scratch/xyang/build-akantu/build/friction-energies-steady_state_" + filename + ".csv"
    ...
    #u = lower_block.point_data["displacement"]

    grad_mesh = lower_block.compute_derivative(
    scalars="displacement",
    gradient=True,
    preference="point"
    )

    grad_u = grad_mesh.point_data["gradient"]
    ...
    for i in range(len(eps)):
        tr = np.trace(eps[i])
        stress[i] = lam * tr * np.eye(3) + 2 * mu * eps[i]
    
    stress_int = stress[lower_interface_ids]
    press_int = stress_int[:,1,1]
    shear_int = stress_int[:,0,0]
    press_int = press_int[order_l]
    shear_int = shear_int[order_l]
    #print(press_int)
    #print(shear_int)
    ...
    plot = ax_interface.plot(lower_x[order_l], shear_int/press_int,linewidth=  1)[0]
    ...
    def update(frame):
        ...
        plot.set_ydata(shear_int/press_int)
    ...
    outfile = f"interface_synctrac_{basename}.mp4"
