...
def main(filename):
    ...
    plot = ax.plot(lower_x[order_l], lower_v_t[order_l] - upper_v_t[order_u],linewidth=  1)[0]
    ...
    ax.set_xlim(lower_x.min(), lower_x.max())
    ax.set_ylim(-4e-1, 2e-2)
    ...
    def update(frame):
        ...
        ax.set_title(
            f"Relative shear velocity   t = {reader.time_values[frame]:.4e}"
        )
    ...
    outfile = f"interface_velocity_{basename}.mp4"
